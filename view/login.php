      ...
      <div class="col-span-5 md:col-span-3 flex items-center justify-center">
        <form class="w-[320px]" action="../controllers/Users.php" method="post">
          ...
          <div>
            ...
            <p class="text-sm mt-1 text-muted-foreground">welcome back, we are happy to use your web.</p>
          </div>
          <div class="space-6 mt-6">
            <div class="space-y-4">
              ...
              <div class="relative">
                ...
                <input class="border pl-10 border-border rounded-md px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" placeholder="Email" id="email" type="email" name="email" value="<?php if (isset($_COOKIE['email'])) echo $_COOKIE['email']; else echo ""; ?>">
              </div>
              <div class="relative">
                <i data-lucide="lock-keyhole" class="w-7 h-7 p-1 absolute left-1.5 top-1/2 transform -translate-y-1/2"></i>
                <input class="border border-border rounded-md px-10 py-2 w-full focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" placeholder="Password" id="Password" type="password" name="password" value="<?php if (isset($_COOKIE['password'])) echo $_COOKIE['password']; else echo ""; ?>">
                <i data-lucide="eye" class="w-7 h-7 p-1 absolute right-1.5 top-1/2 transform -translate-y-1/2 cursor-pointer"></i>
              </div>
              <div class="flex justify-between items-center gap-1.5">
                <div>
                  <input type="checkbox" name="remember" <?php if (isset($_COOKIE['password'])) echo "checked"; else echo ""; ?>  id="remember" class="">
                  <label class="text-sm" for="remember">Remember me</label>
                </div>
                <a class="text-primary text-sm" href="./forgot-password.php">Forgot Password</a>
              </div>
              <button class="text-center mt-7 px-3 py-1 w-full block rounded-md bg-primary text-white cursor-pointer hover:bg-primary/80">
                Login
              </button>
              <p class="text-sm mt-5 text-muted-foreground">Don't have an account? <a class="text-primary" href="./register.php">Register</a></p>
            </div>
          </div>
        </form>
      </div>
